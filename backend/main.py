...
def call_fast_interview_api(prompt: str) -> str:
    """
    Keep interactive interview scoring responsive by trying the faster
    fallback model first and failing over quickly.
    """
    primary_key = os.getenv("DEEPSEEK_API_KEY")
    primary_url = os.getenv("DEEPSEEK_API_URL", "https://api.deepseek.com/v1/chat/completions")
    ...
    if fallback_key and fallback_url:
        attempts.append(("interview fallback", fallback_key, fallback_url, fallback_model, 4))
    if primary_key and primary_key != "your-deepseek-api-key":
        attempts.append(("interview primary", primary_key, primary_url, "deepseek-ai/DeepSeek-V4-Pro", 3))
...
INTERVIEW_QUESTION_POOLS = {
    ...
    "ai": {
        ...
        "project": [
            ...
            "请复盘一次你把模型能力接进真实应用的过程，最难的工程约束是什么？"
        ],
        "scenario": [
    ...
    "lowAltitude": {
        ...
        "project": [
            ...
            "如果要复盘一次试运营，你会设计哪些指标来判断航线是否值得继续投入？",
    ...
    "pm": {
        ...
        "project": [
            "讲一次你通过用户反馈或数据复盘改变产品方案的经历。",
            "如果研发说方案能做但上线维护成本高，你会怎样拆 MVP 和后续版本？",
            "请复盘一个你写过的需求，从问题定义到验证闭环哪里最容易失真？"
        ],
        "scenario": [
...
INTERVIEW_FOLLOW_UP_FALLBACKS = {
    ...
    "lowAltitude": [
        "你刚才提到运营方案，请说一个现场异常或安全风险，你会怎样做预案和复盘？",
...
def build_fallback_follow_up(job_type: str, answer: str, next_turn: int) -> str:
    ...
    if not any(marker in answer for marker in ["%", "指标", "提升", "降低", "结果", "评测", "验证", "复盘"]):
...
3. 如果回答较强，追问 trade-off、失败复盘、业务落地或上线标准。
...
INTERVIEW_DYNAMIC_FOCUS = {
    ...
    "lowAltitude": "低空经济运营要看场景获客与资源入口、航线/服务产品设计、飞行交付、合规安全、异常响应和复盘数据。",
...
def assess_interview_answer_signal(answer: str) -> dict:
    text = answer.strip()
    lower = text.lower()
    evidence_markers = ["%", "提升", "降低", "增长", "减少", "数据", "指标", "用户", "成本", "时延", "准确率", "上线", "复盘", "验证", "ROI", "roi"]
    structure_markers = ["首先", "其次", "最后", "第一", "第二", "第三", "背景", "目标", "行动", "结果", "复盘", "STAR", "star"]
...
3. 如果回答较强，就跳到更高压的胜任自证、预判、复盘、情商或薪资谈判。
...
def generate_rule_based_interview_feedback(question: str, answer: str, job_type: str):
    text = answer.strip()
    length = len(text)
    structure_markers = ["首先", "其次", "最后", "第一", "第二", "第三", "STAR", "背景", "任务", "行动", "结果", "指标", "复盘"]
    evidence_markers = ["%", "提升", "降低", "用户", "数据", "指标", "准确率", "时延", "成本", "上线", "复盘", "验证"]
    job_keywords = {
        "robot": ["ROS", "ROS2", "传感器", "标定", "定位", "规划", "控制", "仿真", "日志", "实时"],
        "ai": ["数据", "评测", "模型", "RAG", "微调", "召回", "指标", "推理", "成本", "安全"],
        "lowAltitude": ["合规", "空域", "航线", "气象", "安全", "运营", "应急", "起降", "准点", "复盘"],
    ...
    comments = [
        ...
        "表达有一定层次，面试官能跟上你的思路。" if scores[1] >= 7 else "建议用“先结论、再展开、最后复盘”的结构组织答案。",
    ...
    if avg_score >= 8:
        ...
        suggestion = "建议按 STAR 重组：先给一句结论，再讲具体项目动作，最后用数据或复盘收束。"
    else:
...
def generate_rule_based_interview_feedback(question: str, answer: str, job_type: str):
    ...
    has_detail = any(k in text for k in ["我负责", "具体", "当时", "问题", "动作", "复盘", "如果重来", "优化前", "优化后"])
    ...
    has_eq = any(k in text for k in ["我理解", "先确认", "沟通", "方案", "共赢", "评估期", "绩效", "申请", "冷静", "复盘"])
    ...
    comments = [
        ...
        "有一定细节和复盘意识，继续补充个人动作与前后指标。" if scores[1] >= 7 else "案例颗粒度不足，需要讲清具体动作、失败点和重来会改哪一步。",
    ...
    suggestion = f"下一轮重点补强「{weakest}」。请把回答改成：先给结论，再给真实场景和个人动作，最后用指标、复盘或沟通方案证明价值感、信任感和匹配度。"
...
STRICT_JOB_KEYWORDS = {
    ...
    "lowAltitude": ["低空", "航线", "运营", "合规", "安全", "空域", "气象", "起降", "物流", "复盘"],
...
def _strict_zero_feedback(reason: str, suggestion: str = "请重新围绕面试问题作答，补充具体项目、个人动作和可验证结果。") -> dict:
    return {
        "score": 0,
        "is_relevant": False,
        "strict_reason": reason,
        "dimensions": [
            _score_dimension("问题相关性", 0, reason),
            _score_dimension("内容准确性", 0, "回答无法证明对问题的理解。"),
            _score_dimension("结构完整性", 0, "没有形成有效面试回答结构。"),
            _score_dimension("证据与细节", 0, "没有提供可验证的项目、数据或行动细节。"),
            _score_dimension("岗位匹配度", 0, "没有体现与目标岗位的匹配。"),
        ],
        "suggestion": suggestion,
    }
...
def _assess_strict_answer(question: str, answer: str, job_type: str) -> dict:
    ...
    if any(marker in text for marker in STRICT_BAD_ANSWER_MARKERS):
...
def generate_rule_based_interview_feedback(question: str, answer: str, job_type: str):
    ...
    dimensions = [
        ...
        _score_dimension(
            ...
            "回答有基本结构。" if assessment["structure"] >= 6 else "结构偏散，建议使用“结论-动作-结果-复盘”。",
        ),
        _score_dimension(
            "证据与细节",
            assessment["evidence"],
    ...
    suggestion = (
        ...
        "量化结果或复盘结论；如果没有项目，也要说明方法、边界和验证方式。"
    )
...
只输出 JSON，不要输出其他内容。"""
    ...
    if os.getenv("INTERVIEW_AI_SCORING", "false").lower() == "true":
        result = call_fast_interview_api(prompt)
...
def build_interview_advice(interview: dict, avg_score: float) -> str:
    ...
    if avg_score >= 8:
        summary = "整体表现优秀，已经能体现岗位相关能力；下一步重点是把亮点讲得更有证据、更像真实业务复盘。"
    elif avg_score >= 6:
    ...
    dimension_metaphors = {
        ...
        "细节与迭代能力": "像有经验的项目参与者，关键是把复盘颗粒度讲到动作级。",
    ...
    return "\n".join([
        ...
        f"建议1：围绕{job_names.get(job_type, '目标岗位')}的真实考察点重构答案，重点补上{job_focus.get(job_type, job_focus['ai'])}。每个答案至少包含“业务背景/任务目标/你的动作/量化结果/复盘改进”五个信息块。",
